...
Fixes vs original:
  - reasoning[:120] in print_trail was silently cutting off short strings
    that are already under 120 chars — no functional issue but now handled cleanly.
  - confidence field was typed Any in original; tightened to str.
"""
...
class AuditLog:
    """
    Append-only audit trail. Every agent decision recorded here.
    ...
    def print_trail(self) -> None:
        ...
        for e in self.entries:
            snippet = e.reasoning[:120] + ("..." if len(e.reasoning) > 120 else "")
            print(f"\n  Step {e.step} [{e.agent}]")
            ...
            if e.citations:
                print(f"  Citations : {', '.join(str(c) for c in e.citations)}")
        print(f"\n{'='*60}\n")
